...
class HtmlWebpackPluginUpdated {
    ...
    apply(compiler) {
        ...
        const childCompilerPromises = [];

        this.htmlWebpackPlugins.forEach((plugin) => {
            plugin.options.template = this.getFullTemplatePath(plugin.options.template, compiler.context);

            // convert absolute filename into relative so that webpack can
            // generate it at correct location
            const filename = plugin.options.filename;
            if (path.resolve(filename) === path.normalize(filename)) {
                plugin.options.filename = path.relative(compiler.options.output.path, filename);
            }
            ...
            plugin.options.filename = plugin.options.filename.replace(/\[(?:(\w+):)?contenthash(?::([a-z]+\d*))?(?::(\d+))?\]/ig, (match) => {
            ...
            const minify = plugin.options.minify;
        ...
        compiler.hooks.thisCompilation.tap('HtmlWebpackPlugin', (compilation) => {
            debugger;
            console.info('hook thisCompilation has been called')
            ...
            compilation.hooks.additionalChunkAssets.tap('HtmlWebpackPlugin', () => {
                debugger;
                console.log('hook add additionalChunkAssets started')
                ...
                console.log('hook add additionalChunkAssets finished')
            });
            console.info('hook thisCompilation has finished ')
        });

        compiler.hooks.make.tapAsync('HtmlWebpackPlugin', (compilation, callback) => {
            console.info('hook make has been called')
            this.htmlWebpackPlugins.forEach((plugin) => {
                debugger;
                const childCompilerPromise = childCompiler.compileTemplate(plugin.options.template, plugin.options.filename, compilation)
                    .catch(err => {
                ...
                childCompilerPromises.push(childCompilerPromise);
            });

            Promise.all(childCompilerPromises)
                .then((results) => {
                    const a = results;
                    console.info('hook make finished')
                    debugger;
                    callback();
                })
        });
