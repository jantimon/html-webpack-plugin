...
- `inject`: `true | 'head' | 'body' | false` | `RegExp` | `Function` Inject all assets into the given `template` or `templateContent` - When passing `true` or `'body'` all javascript resources will be placed at the bottom of the body element. `'head'` will place the scripts in the head element. If a RegExp is passed in, the script that passes the RegExp is placed at the bottom of the body, and if not, it will be placed at the bottom of the head. If a function is passed in the same way, the url of the script will be used as a parameter.
