...
function hasOutDatedTemplateCache (mainCompilation, callback) {
  ...
  function updateSnapshot (snapshotValidErr, snapshotValid) {
    return mainCompilation.fileSystemInfo.createSnapshot(
      Date.now(),
      ...
      (err, snapshot) => {
        ...
        snapshotMap.set(childCompiler, snapshot);
        if (isChildCompilerCacheOutdated(childCompiler, snapshot)) {
          return callback(null, false);
        } else {
          return callback(snapshotValidErr, snapshotValid);
        }
      }
    );
  }
  ...
  const prevSnapshot = snapshotMap.get(childCompiler);
  if (prevSnapshot !== undefined) {
    return mainCompilation.fileSystemInfo.checkSnapshotValid(
      prevSnapshot,
      (err, isValid) => {
        return updateSnapshot(err, isValid);
      }
    );
  }

  return updateSnapshot(null, false);
}
 ...
 * @returns {boolean}
 */
function isChildCompilerCacheOutdated (childCompiler, snapshot) {
  ...
  const isCacheOutOfDate = childCompiler.fileDependencies
    .some((fileDependency) => {
      const timestamp = fileTimestamps.get(fileDependency).timestamp;
      // Snapshot timestamps are known unless it is a directory in webpack5
      // If the timestamp is larger then the file has changed
      // Otherwise the file is still the same
      console.log(fileDependency, fileTimestamps.get(fileDependency), timestamp ? timestamp > childCompiler.compilationStartedTimestamp : false);
      return timestamp ? timestamp > childCompiler.compilationStartedTimestamp : false;
    });
