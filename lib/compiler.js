 ...
 * @type {WeakMap<WebpackCompilation, boolean>}}
 */
const compilationNeedsRebuildCache = new WeakMap();

/**
 * @type {number}
 */
let previousCompilationTimestamp = Date.now();
 ...
 */
function needsRebuild (compilation) {
  // If response is cached, don't calculate again
  let needsRebuild = compilationNeedsRebuildCache.get(compilation);
  if (needsRebuild !== undefined) {
    return needsRebuild;
  }
  needsRebuild = false;
  const childCompiler = getChildCompiler(compilation.compiler);
  const fileTimestamps = compilation.fileTimestamps;
  // If no file timestamps, this is probably the first compilation
  // Always rebuild for first compilation
  if (fileTimestamps.size < 1) {
    needsRebuild = true;
  }
  childCompiler.fileDependencies.forEach(file => {
    const lastModified = fileTimestamps.get(file);
    if (!lastModified) {
      return; // Next file
    }
    // Checks if file has changed since previous compilation
    if (lastModified > previousCompilationTimestamp) {
      needsRebuild = true;
    }
  });
  previousCompilationTimestamp = childCompiler.compilationStartedTimestamp;
  ...
  // - @dwoznicki
  clearCache(compilation.compiler);
