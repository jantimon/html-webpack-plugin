...
class HtmlWebpackChildCompiler {
  ...
  constructor () {
    ...
    this.templates = [];
    /**
     * @type {Promise<{[templatePath: string]: { content: string, hash: string, entry: WebpackChunk }}> | undefined}
     */
    ...
    /**
     * Used to determine if a file has changed since the previous compilation
     * @type {number}
     */
    this.previousCompilationStartedTimestamp = Date.now();
    /**
  ...
  /**
   * Returns `true` if any template file depenendencies has changed
  ...
  hasOutdatedTemplateCache (mainCompilation) {
    ...
    // If the compilation was never run the templates need rebuilding
    if (!this.compilationStartedTimestamp || fileTimestamps.size < 1) {
      this.cacheVerifiedForCompilation.set(mainCompilation, true);
      return true;
    }
    ...
    const isCacheOutOfDate = this.fileDependencies.some((fileDependency) => {
      ...
      return !timestamp || timestamp > this.previousCompilationStartedTimestamp;
    });
    this.cacheVerifiedForCompilation.set(mainCompilation, isCacheOutOfDate);
    this.previousCompilationStartedTimestamp = this.compilationStartedTimestamp;
...
function clearCachedPromise (mainCompiler) {
  const childCompiler = childCompilerCache.get(mainCompiler);
  if (childCompiler) {
    childCompiler.compilationPromise = undefined;
  }
}
...
/**
 * Returns `true` if template file and/or other dependencies have been modified
 * since the last compilation.
 * @param {WebpackCompilation} compilation
 */
function hasOutdatedTemplateCache (compilation) {
  const childCompiler = childCompilerCache.get(compilation.compiler);
  return childCompiler ? childCompiler.hasOutdatedTemplateCache(compilation) : true;
}
