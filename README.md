...
|**[`chunks`](#)**|`{Array.<string>\|RegExp\|Function}`|``|Allows you to add only some chunks (e.g only the unit-test chunk)|
...
Also, you can match by Function
...
plugins: [
  new HtmlWebpackPlugin({
    chunks: function(chunkName) {
      if (chunkName === 'app') {
        return true; // If you want to include the chunk, you should return exact `true`
      }
      return false;
    }
  })
...
Also you can match by RegExp or Function, which is as same as Include
